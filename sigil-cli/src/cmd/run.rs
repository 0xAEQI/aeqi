...
use crate::helpers::{
    augment_identity_with_org_context, build_project_tools, build_provider, build_tools,
    find_agent_dir, find_project_dir, load_config, open_memory,
};
...
pub(crate) async fn cmd_run(
    ...
    let model = model_override
        ...
        .unwrap_or_else(|| config.model_for_agent(config.leader()));
    ...
    let identity = if let Some(rn) = project_name {
        ...
        let agent_dir = find_agent_dir(config.leader()).ok();
        ...
        find_agent_dir(config.leader())
            .ok()
            .map(|d| Identity::load(&d, None).unwrap_or_default())
            .unwrap_or_default()
    };
    let identity =
        augment_identity_with_org_context(&config, identity, Some(config.leader()), project_name);
