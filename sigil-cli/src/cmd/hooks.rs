...
fn scripts_dir() -> PathBuf {
    PathBuf::from("/home/claudedev/sigil/scripts")
}
...
pub(crate) async fn cmd_hooks_test(
    ...
    let default_input =
        r#"{"file_path":"/home/claudedev/sigil/crates/sigil-core/src/lib.rs"}"#.to_string();
...
pub(crate) async fn cmd_hooks_bench(script: Option<&str>, iterations: u32) -> Result<()> {
    ...
    let home = std::env::var("HOME").context("HOME not set")?;
    let gate = PathBuf::from(&home).join(".sigil/session/recall.gate");
