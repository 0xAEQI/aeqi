...
/* ─── Hero ─── */
function Hero({ ready, onLogoReady }: { ready: boolean; onLogoReady: () => void }) {
  ...
  return (
    <section className="flex-1 flex items-center justify-center px-6">
      <div className="max-w-3xl mx-auto text-center py-32">
        {/* Logo always renders — particles fly in as the first thing */}
        <div className="flex justify-center">
          <Suspense fallback={
            <span className="text-[120px] md:text-[180px] font-bold tracking-tighter leading-none text-black/60 select-none">æ</span>
          }>
            <ParticleLogo size={300} onReady={onLogoReady} />
          </Suspense>
        </div>

        {/* Content fades in after particles settle */}
        <AnimatePresence>
          {ready && (
            <>
              <motion.p
...
export default function App() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      <Nav visible={logoReady} />
      <Hero ready={logoReady} onLogoReady={handleLogoReady} />

      <AnimatePresence>
        {logoReady && (
          <motion.div
            className="bg-black/[0.02] flex-1 flex flex-col"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
