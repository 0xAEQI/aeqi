...
use sigil_beads::{BeadStore, Priority};
use std::path::PathBuf;
use std::sync::Mutex;

/// Tool for creating beads (tasks).
pub struct BeadsCreateTool {
    store: Mutex<BeadStore>,
    prefix: String,
}

impl BeadsCreateTool {
    pub fn new(beads_dir: PathBuf, prefix: String) -> Result<Self> {
        let store = BeadStore::open(&beads_dir)?;
    ...
    async fn execute(&self, args: serde_json::Value) -> Result<ToolResult> {
        ...
        let bead = store.update(id, |b| {
            if let Some(s) = status {
                b.status = match s {
                    "in_progress" => sigil_beads::BeadStatus::InProgress,
                    "done" => sigil_beads::BeadStatus::Done,
                    "blocked" => sigil_beads::BeadStatus::Blocked,
                    "cancelled" => sigil_beads::BeadStatus::Cancelled,
                    _ => sigil_beads::BeadStatus::Pending,
                };
            }
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec {
            ...
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "id": { "type": "string", "description": "Bead ID (e.g. as-001)" },
...
impl Tool for BeadsShowTool {
    async fn execute(&self, args: serde_json::Value) -> Result<ToolResult> {
        ...
        if let Some(bead) = store.get(id) {
            ...
            Ok(ToolResult::error(format!("Bead not found: {id}")))
        }
    }
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec {
            ...
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "id": { "type": "string", "description": "Bead that will be blocked" },
                    "depends_on": { "type": "string", "description": "Bead that must complete first" }
                },
