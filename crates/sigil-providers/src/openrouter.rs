...
impl OpenRouterProvider {
    ...
    pub async fn generate_image(&self, prompt: &str, model: &str) -> Result<Vec<u8>> {
        ...
        let response = self
            ...
            .header("HTTP-Referer", "https://github.com/USER/sigil")
            .header("X-Title", "Sigil")
            .json(&api_request)
            .send()
            .await
...
fn convert_messages(messages: &[sigil_core::traits::Message]) -> Vec<ApiMessage> {
    use sigil_core::traits::{ContentPart, MessageContent, Role};
