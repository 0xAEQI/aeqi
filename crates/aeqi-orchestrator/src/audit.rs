...
impl AuditLog {
    ...
    pub fn open(path: &Path) -> Result<Self> {
        ...
        conn.execute_batch(
            "PRAGMA journal_mode=WAL;
             ...
             CREATE TABLE IF NOT EXISTS aeqi_decisions (
             ...
             CREATE INDEX IF NOT EXISTS idx_decisions_task
                 ON aeqi_decisions(task_id);
             CREATE INDEX IF NOT EXISTS idx_decisions_project
                 ON aeqi_decisions(project);
             CREATE INDEX IF NOT EXISTS idx_decisions_timestamp
                 ON aeqi_decisions(timestamp);",
        )?;
    ...
    pub fn record(&self, event: &AuditEvent) -> Result<()> {
        ...
        conn.execute(
            "INSERT INTO aeqi_decisions (timestamp, project, task_id, decision_type, agent, reasoning, metadata_json)
    ...
    pub fn query_by_task(&self, task_id: &str) -> Result<Vec<AuditEvent>> {
        ...
        Self::query_events(
            ...
            "SELECT timestamp, project, task_id, decision_type, agent, reasoning, metadata_json
             FROM aeqi_decisions WHERE task_id = ?1 ORDER BY timestamp ASC",
    ...
    pub fn query_by_project(&self, project: &str) -> Result<Vec<AuditEvent>> {
        ...
        Self::query_events(
            &conn,
            "SELECT timestamp, project, task_id, decision_type, agent, reasoning, metadata_json
             FROM aeqi_decisions WHERE project = ?1 ORDER BY timestamp ASC",
    ...
    pub fn query_recent(&self, limit: u32) -> Result<Vec<AuditEvent>> {
        ...
        Self::query_events(
            &conn,
            "SELECT timestamp, project, task_id, decision_type, agent, reasoning, metadata_json
             FROM aeqi_decisions ORDER BY timestamp DESC LIMIT ?1",
