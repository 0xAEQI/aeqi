...
pub struct WorkerPool {
    ...
    pub model: String,
    pub identity: aeqi_core::Identity,
    ...
    pub metrics: Option<Arc<AEQIMetrics>>,
    ...
    pub execution_mode: aeqi_core::ExecutionMode,
...
impl WorkerPool {
    ...
    fn load_skill(&self, skill_name: &str) -> Option<aeqi_tools::Skill> {
        for dir in &self.skills_dirs {
            let path = dir.join(format!("{skill_name}.toml"));
            if path.exists() {
                match aeqi_tools::Skill::load(&path) {
    ...
    /// Create a native AEQI worker for a task.
    async fn create_worker(
        &self,
        agent_name: String,
        worker_name: String,
        task: &aeqi_tasks::Task,
    ) -> AgentWorker {
        ...
        let mut worker = match self.execution_mode {
            aeqi_core::ExecutionMode::ClaudeCode => {
                let cwd = self
                    .repo
                    .clone()
                                    ...
                                    serde_json::from_str::<aeqi_core::SessionState>(&content)
                            {
                                ...
                                for msg in &state.messages {
                                    let role = match msg.role {
                                        ...
                                        aeqi_core::traits::Role::Assistant => "assistant",
                                        aeqi_core::traits::Role::System => "system",
    ...
    async fn attempt_project_resolution(&self, task: &aeqi_tasks::Task, current_depth: u32) {
    ...
    async fn escalate_to_leader(&self, task: &aeqi_tasks::Task) {
    ...
    pub async fn spawn_worker(&mut self, task: &aeqi_tasks::Task, agent_name: &str) {
    ...
    pub async fn ready_for_spawn(&self) -> Vec<(aeqi_tasks::Task, String)> {
        if self.paused {
            return vec![];
        }
        // Budget check.
