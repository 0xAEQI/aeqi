...
impl ExpertiseLedger {
    pub fn open(path: &Path) -> Result<Self> {
        ...
        conn.execute_batch(
            "PRAGMA journal_mode=WAL;
             ...
             CREATE TABLE IF NOT EXISTS aeqi_expertise (
             ...
             CREATE INDEX IF NOT EXISTS idx_expertise_agent_domain
                 ON aeqi_expertise(agent_name, task_domain);
             CREATE INDEX IF NOT EXISTS idx_expertise_domain
                 ON aeqi_expertise(task_domain);",
        )?;
    ...
    pub fn record(&self, entry: &ExpertiseRecord) -> Result<()> {
        ...
        conn.execute(
            "INSERT INTO aeqi_expertise (agent_name, task_domain, outcome, cost_usd, duration_secs, turns, timestamp)
    ...
    pub fn is_deprioritized(&self, agent: &str, domain: &str) -> Result<bool> {
        ...
        let (total, failures): (u32, u32) = conn
            .query_row(
             ...
             FROM aeqi_expertise WHERE agent_name = ?1 AND task_domain = ?2",
