...
impl GraphGuardrailsMiddleware {
    ...
    fn check_edit_impact(&self, project: &str, file_path: &str, input: &str) -> Option<String> {
        ...
        let store = sigil_graph::GraphStore::open(&db_path).ok()?;
        ...
        for node in &nodes {
            if matches!(
                node.label,
                sigil_graph::NodeLabel::File
                    | sigil_graph::NodeLabel::Module
                    | sigil_graph::NodeLabel::Community
                    | sigil_graph::NodeLabel::Process
            ) {
                continue;
            }
            ...
            let ext_caller_count = incoming
                .iter()
                .filter(|(e, caller)| {
                    e.edge_type == sigil_graph::EdgeType::Calls
                        && caller
                            .as_ref()
            ...
            if node.label == sigil_graph::NodeLabel::Trait {
                let impl_count = incoming
                    .iter()
                    .filter(|(e, _)| e.edge_type == sigil_graph::EdgeType::Implements)
                    .count();
                if impl_count >= 2 {
                    high_impact.push(format!(
