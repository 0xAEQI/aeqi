...
if [ -f "$EDITS_LOG" ] && [ -S "$SOCK" ]; then
    ...
    if [ "$DISTINCT_FILES" -gt 5 ]; then
        printf '{"cmd":"post_blackboard","project":"%s","key":"signal:primer-update-nudge:%s","content":"Previous session edited %s distinct files in %s. The project primer (AEQI.md) may need updating.","tags":["nudge","primer"],"durability":"transient"}' \
