# AEQI vs Hermes, Supermemory, and Deer Flow

Date: 2026-03-25
Current AEQI revision: `a517e2a26cb1`
...
- Current local AEQI passed `cargo test --workspace -q`
- Total locally passing tests from the run: 433

## Purpose

This document is the first direct comparison pass against the current AEQI v4 codebase.

The goal is not to decide which repo is "coolest."
The goal is to define how AEQI becomes the strongest system across all of the important angles:
...
Current AEQI is now the strongest orchestration architecture in this set.

That was not fully true before v4. It is much more defensible now.

The reason is that current AEQI has a real layered system:
...
- AEQI leads on orchestration control plane design
...
That means AEQI does not need to copy any of them wholesale.
It needs to beat each one on their own angle while preserving its stronger core.

## What AEQI Already Wins

### 1. Orchestration as a first-class system

AEQI is now the clearest "real orchestrator" of the group.
...
Current AEQI has an explicit verification pipeline in `crates/aeqi-orchestrator/src/verification.rs`.
...
The gap is no longer "AEQI lacks memory depth."
The gap is now mostly "AEQI lacks memory product surfaces and integrations."

### 4. Notes/directives are a real differentiator

The notes system in `crates/aeqi-orchestrator/src/notes.rs` is one of the clearest AEQI-native moats.
...
Current AEQI's proactive layer is not just "cron plus agent."
...
AEQI needs to beat Hermes by importing these strengths:

### What AEQI must take from Hermes
...
### What AEQI must not copy from Hermes
...
## Where Supermemory Still Beats AEQI
...
AEQI now has more interesting memory internals than before, but Supermemory is still better at turning memory into a clean external product.

### What AEQI must take from Supermemory
...
### What AEQI must not copy from Supermemory

- memory-as-intelligence reductionism
- dependence on opaque externalized core behavior for AEQI's main truth
- over-optimizing for integration wrappers while neglecting orchestration depth

## Where Deer Flow Still Beats AEQI
...
AEQI is deeper as an orchestrator, but Deer Flow is ahead in the experience of interacting with agent execution in real time.

### What AEQI must take from Deer Flow
...
### What AEQI must not copy from Deer Flow
...
Most interesting current system: AEQI
...
## What "Best Possible AEQI" Actually Means

The best possible AEQI is not:

- Hermes plus tasks
- Supermemory plus routing
- Deer Flow plus a daemon

The best possible AEQI is:

- AEQI's orchestration core
...
### Track 1. Defend and deepen AEQI's native moat
...
This is the core that must stay uniquely AEQI.
...
AEQI becomes as reliable to operate daily as Hermes, while remaining much stronger in orchestration.

### Track 3. Beat Supermemory on memory product

Current AEQI now has enough internal memory depth that this is realistic.
...
AEQI keeps deeper orchestration-linked memory than Supermemory, but becomes just as easy to consume.
...
AEQI's live experience becomes as legible and satisfying as Deer Flow's, but grounded in a stronger control plane.

### Track 5. Prove the claim

This is still mandatory.

Even with v4, AEQI still needs stronger proof than nouns.
...
AEQI is not only more ambitious than the others. It is measurably better.
...
"AEQI has ideas but not enough system."

It is now:

"AEQI has a strong system, but it may still lose in product reality to narrower tools that feel easier, safer, and more complete."

That is the real competitive danger.

Hermes can beat AEQI in daily usability.
Supermemory can beat AEQI in memory usability.
Deer Flow can beat AEQI in live execution UX.

So AEQI must not only be deeper.
It must become equally usable.

## Final Assessment

Current AEQI is now credible as the strongest orchestration architecture in this comparison set.
...
To become the best possible system, AEQI must now do three things:
...
If that happens, AEQI will not just be better at "orchestration."
