...
> Recursive multi-agent orchestration framework in Rust. A single ~7MB binary (`sigil`).

**Key stats**: 8 Rust crates, 26 orchestrator modules, 145 tests, zero clippy warnings.
...
Sigil is a multi-agent orchestration framework where **workers ARE orchestrators**. Each worker runs as a full Claude Code instance with Task tool access, enabling recursive sub-agent spawning. The system is designed around trait-driven swappability, budget-gated execution, and external observation over self-reporting.
...
|-------|------|---------|-------------|
| `sigil-core` | `crates/sigil-core/` | Foundation: traits, config, agent loop, security, identity | `SigilConfig`, `Identity`, `SecretStore`, `Agent`, 6 core traits |
| `sigil-tasks` | `crates/sigil-tasks/` | Git-native task DAG with JSONL storage | `TaskBoard`, `Task`, `TaskId`, `Mission`, `Priority`, `TaskStatus` |
| `sigil-providers` | `crates/sigil-providers/` | LLM provider implementations + cost estimation | `OpenRouterProvider`, `AnthropicProvider`, `OllamaProvider`, `ReliableProvider`, `OpenRouterEmbedder`, `estimate_cost` |
| `sigil-memory` | `crates/sigil-memory/` | Hybrid search: SQLite FTS5 + vector + temporal decay | `SqliteMemory`, `VectorStore`, `merge_scores`, `mmr_rerank`, `apply_decay`, `chunk_text` |
| `sigil-tools` | `crates/sigil-tools/` | Tool implementations for the agent loop | `ShellTool`, `FileReadTool`, `FileWriteTool`, `GitWorktreeTool`, `DelegateTool`, `BeadsCreateTool`, `Skill` |
| `sigil-gates` | `crates/sigil-gates/` | Messaging channel implementations | `TelegramChannel`, `DiscordChannel`, `SlackChannel` |
| `sigil-orchestrator` | `crates/sigil-orchestrator/` | The brain: 25 modules for full orchestration | `Daemon`, `Supervisor`, `AgentWorker`, `AgentRouter`, `ClaudeCodeExecutor`, `ProjectRegistry`, `CostLedger`, `DispatchBus`, `SigilMetrics`, `Council`, `Reflection`, `GapAnalyzer`, `EmotionalState`, `AgentCheckpoint` |
| `sigil` | `sigil-cli/src/main.rs` | CLI binary entry point | `Cli`, `Commands` enum |

---

## 3. Core Traits

All 6 traits are defined in `crates/sigil-core/src/traits/` and re-exported from `sigil_core::traits`.

### Provider

```rust
// crates/sigil-core/src/traits/provider.rs
#[async_trait]
...
```rust
// crates/sigil-core/src/traits/tool.rs
#[async_trait]
...
**Implementations**: 13 in `sigil-tools` (Shell, FileRead, FileWrite, ListDir, GitWorktree, Delegate, Porkbun, 6x Beads), 13 in `sigil-orchestrator` (ProjectStatus, ProjectAssign, ProjectList, MailRead, MailSend, AllReady, ChannelReply, UsageStats, MemoryStore, MemoryRecall, QuestDetail, QuestCancel, QuestReprioritize)

### Memory

```rust
// crates/sigil-core/src/traits/memory.rs
#[async_trait]
pub trait Memory: Send + Sync {
    async fn store(&self, key: &str, content: &str, category: MemoryCategory,
                   scope: MemoryScope, entity_id: Option<&str>) -> Result<String>;
...
**Enums**: `MemoryCategory` (`Fact`, `Procedure`, `Preference`, `Context`, `Evergreen`), `MemoryScope` (`Entity` (entity-scoped), `Domain` (project-scoped), `System` (global))
...
```rust
// crates/sigil-core/src/traits/observer.rs
#[async_trait]
...
**Implementations**: `LogObserver` (tracing), `PrometheusObserver` (atomic counters) — both in `sigil-core`

### Channel

```rust
// crates/sigil-core/src/traits/channel.rs
#[async_trait]
...
```rust
// crates/sigil-core/src/traits/embedder.rs
#[async_trait]
...
Configuration lives at `config/sigil.toml`. Discovered via `SigilConfig::discover()`:

1. Check `$SIGIL_CONFIG` env var
2. Walk upward from CWD looking for `sigil.toml` (also in `config/` subdirectory)
...
project-alpha = "/home/user/project-alpha-repo"
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `leader` | String | `"leader"` | System leader agent name |
...
`SigilConfig::validate()` checks:
- Non-empty `sigil.name`
...
**File**: `crates/sigil-core/src/identity.rs`
...
2. Resolves `projects/{project_name}/` for project context
...
7. `# User Preferences` — from `preferences` (budget: 4,000 chars)
...
sequenceDiagram
    ...
    participant CLI as sigil assign
    ...
    Human->>CLI: sigil assign "Fix the bug" --project project-alpha
    CLI->>TB: create_task("mp-042", "Fix the bug")
    ...
    Supervisor->>Supervisor: cost_ledger.can_afford_project("project-alpha")?
    ...
    Worker->>Worker: Build task_context (subject + description + checkpoints + acceptance criteria)
    Worker->>Worker: Inject recalled memories (up to 5 entries)
    Worker->>Worker: Enrich identity with dynamic memory recall

    Worker->>Executor: execute(identity, task_context)
    Executor->>Executor: ContextBudget.apply_to_identity() → system prompt
    Executor->>Executor: Append WORKER_PROTOCOL

    Executor->>Claude: claude -p "task_context"<br/>--output-format json<br/>--permission-mode bypassPermissions<br/>--model claude-sonnet-4-6<br/>--max-turns 25<br/>--append-system-prompt "..."
...
|------|------|-----------------|
| Task creation | `crates/sigil-tasks/src/store.rs` | `TaskBoard::create()` |
| Wake patrol | `crates/sigil-orchestrator/src/registry.rs` | `ProjectRegistry::assign()` → `wake.notify_one()` |
| Parallel patrol | `crates/sigil-orchestrator/src/registry.rs` | `ProjectRegistry::patrol_all()` → `futures::join_all` |
| Supervisor patrol | `crates/sigil-orchestrator/src/supervisor.rs` | `Supervisor::patrol()` — 5 phases |
| Budget check | `crates/sigil-orchestrator/src/cost_ledger.rs` | `CostLedger::can_afford_project()` |
| Worker creation | `crates/sigil-orchestrator/src/supervisor.rs` | `Supervisor::create_worker()` |
| Quest context | `crates/sigil-orchestrator/src/agent_worker.rs` | `AgentWorker::execute()` |
| System prompt | `crates/sigil-orchestrator/src/context_budget.rs` | `ContextBudget::apply_to_identity()` |
| Claude spawn | `crates/sigil-orchestrator/src/executor.rs` | `ClaudeCodeExecutor::execute_once()` |
| Outcome parse | `crates/sigil-orchestrator/src/executor.rs` | `TaskOutcome::parse()` |
| Checkpoint capture | `crates/sigil-orchestrator/src/checkpoint.rs` | `AgentCheckpoint::capture()` |
| Reflection | `crates/sigil-orchestrator/src/agent_worker.rs` | `AgentWorker::reflect_on_result()` |
...
`Agent` mode uses the internal `Agent` struct with `AgentConfig { max_iterations: 20 }` and estimates cost via `sigil_providers::estimate_cost()`.
...
| System leader | — | Send to system leader (leader) | `DispatchKind::Escalation` to leader |
...
**File**: `crates/sigil-orchestrator/src/daemon.rs`
...
**File**: `crates/sigil-orchestrator/src/agent_router.rs`
...
**File**: `crates/sigil-orchestrator/src/council.rs`
...
**SqliteMemory** (`sqlite.rs`): Main `Memory` trait implementation. SQLite database with WAL mode, FTS5 virtual table for full-text search. Tables: `memories` (id, key, content, category, scope, entity_id, embedding BLOB, created_at, accessed_at) and `memories_fts` (FTS5 on key + content).
...
| Scope | Purpose |
|-------|---------|
| `Entity` | Entity-specific memories |
| `Domain` | Project-scoped knowledge |
| `System` | Cross-project knowledge |
...
**File**: `crates/sigil-orchestrator/src/cost_ledger.rs`
...
**File**: `crates/sigil-orchestrator/src/metrics.rs`
...
|------|------|-------------|
| Counter | `sigil_tasks_completed_total` | Total tasks completed |
| Counter | `sigil_tasks_failed_total` | Total tasks failed |
| Counter | `sigil_tasks_blocked_total` | Total tasks blocked |
...
| Name | Buckets |
|------|---------|
| `sigil_worker_duration_seconds` | 10, 30, 60, 120, 300, 600, 1800, 3600 |
| `sigil_task_cost_usd` | 0.01, 0.05, 0.10, 0.25, 0.50, 1.0, 2.0, 5.0, 10.0 |
| `sigil_patrol_cycle_seconds` | 0.01, 0.05, 0.1, 0.5, 1.0, 5.0, 10.0 |

**Per-Project Metrics** (via `ProjectMetrics`):
- `sigil_project_tasks_completed_total{project="<name>"}`
- `sigil_project_tasks_failed_total{project="<name>"}`
- `sigil_project_workers_active{project="<name>"}`
- `sigil_project_cost_usd_total{project="<name>"}`

Rendered via `SigilMetrics::render()` in standard Prometheus text exposition format with `# HELP` and `# TYPE` annotations. Accessible via `sigil daemon query metrics`.

### Dispatch Bus

**File**: `crates/sigil-orchestrator/src/message.rs`
...
**File**: `crates/sigil-orchestrator/src/session_tracker.rs`

Runs in a dedicated `tokio::spawn`, independent from the patrol loop. Sends Telegram notifications on state transitions:

| Event | Message |
|-------|---------|
| Idle → Active | "Workers awakened" |
| Active → Idle | "Queue empty" |
| Periodic check-in | "Sprint check-in: N workers active" |
| Periodic idle alarm | "System idle — ready for your next command" |
...
**File**: `crates/sigil-orchestrator/src/checkpoint.rs`
...
**File**: `crates/sigil-orchestrator/src/reflection.rs`
...
**Tracked files** (read for drift detection): `PERSONA.md`, `IDENTITY.md`, `AGENTS.md`, `HEARTBEAT.md`, `PREFERENCES.md`
...
**File**: `crates/sigil-orchestrator/src/gap_analysis.rs`
...
**File**: `crates/sigil-orchestrator/src/emotional_state.rs`
...
**Files**: `crates/sigil-tasks/src/`

### Hierarchical Task IDs

```
mp-001          ← root task (depth 0)
mp-001.1        ← first child (depth 1)
mp-001.1.3      ← grandchild (depth 2)
```
...
JSONL-backed persistent store. One file per prefix: `.tasks/mp.jsonl`, `.tasks/xx.jsonl`, etc.
...
**File**: `crates/sigil-orchestrator/src/schedule.rs`
...
**File**: `crates/sigil-orchestrator/src/template.rs`
...
**File**: `crates/sigil-orchestrator/src/pipeline.rs`
...
**File**: `crates/sigil-orchestrator/src/operation.rs`

Operations track work that spans multiple projects.

```rust
pub struct Operation {
    pub id: String,                     // "op-{uuid}"
    pub name: String,
...
**File**: `crates/sigil-core/src/security.rs`
...
**CLI**: `sigil secrets set/get/list/delete`
...
Binary: `sigil` — "Sigil — Multi-Agent Orchestration"
...
| Command | Description |
|---------|-------------|
| `sigil init` | Initialize Sigil: creates `config/sigil.toml`, `projects/`, `~/.sigil/` |
| `sigil status` | System-wide status: teams, agents, projects with task counts |
| `sigil doctor [--fix]` | Run diagnostics. `--fix` auto-creates missing dirs/files |
| `sigil config show` | Show loaded configuration |
| `sigil config reload` | Send SIGHUP to daemon for config reload |
| `sigil team [--project NAME]` | Show team assignments (system + per-project) |

### Agent Execution

| Command | Description |
|---------|-------------|
| `sigil run <prompt> [-r PROJECT] [--model MODEL] [--max-iterations N]` | One-shot agent run |

### Task Management

| Command | Description |
|---------|-------------|
| `sigil assign <subject> -r PROJECT [-d DESC] [-p PRIORITY] [-m MISSION]` | Create a task |
| `sigil ready [-r PROJECT]` | Show unblocked tasks |
| `sigil beads [-r PROJECT] [--all]` | List open tasks (or all with `--all`) |
| `sigil close <ID> [-r REASON]` | Close a task |
| `sigil done <ID> [-r REASON]` | Mark done + update operations |
| `sigil hook <WORKER> <TASK_ID>` | Pin task to a specific worker |

### Missions

| Command | Description |
|---------|-------------|
| `sigil mission create <name> -r PROJECT [-d DESC]` | Create a mission |
| `sigil mission list [-r PROJECT] [--all]` | List missions |
| `sigil mission status <ID>` | Mission details with task breakdown |
| `sigil mission close <ID>` | Manually close a mission |
...
| Command | Description |
|---------|-------------|
| `sigil pipelinepour <template> -r PROJECT [--var key=value ...]` | Instantiate a template |
| `sigil pipelinelist [-r PROJECT]` | List available templates |
| `sigil pipelinestatus <ID>` | Pipeline progress |

### Scheduling

| Command | Description |
|---------|-------------|
| `sigil cron add <name> --schedule EXPR -r PROJECT -p PROMPT [--isolated]` | Add cron job |
| `sigil cron add <name> --at ISO8601 -r PROJECT -p PROMPT` | Add one-shot job |
| `sigil cron list` | List all cron jobs |
| `sigil cron remove <name>` | Remove a cron job |

### Skills

| Command | Description |
|---------|-------------|
| `sigil skill list [-r PROJECT]` | List available skills |
| `sigil skill run <name> -r PROJECT [prompt]` | Execute a skill |

### Operations

| Command | Description |
|---------|-------------|
| `sigil operation create <name> <task_ids...>` | Create cross-project operation |
| `sigil operation list` | List active operations |
| `sigil operation status <ID>` | Operation details |

### Memory

| Command | Description |
|---------|-------------|
| `sigil recall <query> [-r PROJECT] [-k TOP_K]` | Search collective memory |
| `sigil remember <key> <content> [-r PROJECT]` | Store a memory |

### Secrets

| Command | Description |
|---------|-------------|
| `sigil secrets set <name> <value>` | Store encrypted secret |
| `sigil secrets get <name>` | Retrieve secret |
| `sigil secrets list` | List secret names |
| `sigil secrets delete <name>` | Delete secret |
...
│
├── crates/
│   ├── sigil-core/                    # Traits, config, agent, security, identity
│   │   └── src/
│   │       ├── lib.rs
│   │       ├── agent.rs                # Agent struct + loop
│   │       ├── config.rs               # SigilConfig + all config structs (~1145 lines)
...
│   │
│   ├── sigil-tasks/                   # Task DAG, JSONL store, missions
│   │   └── src/
│   │       ├── lib.rs
...
│   │       └── query.rs               # TaskQuery
│   │
│   ├── sigil-orchestrator/            # The brain: 25 modules
│   │   └── src/
│   │       ├── lib.rs
...
│   │       ├── metrics.rs              # SigilMetrics (Prometheus)
...
│   │
│   ├── sigil-memory/                  # Hybrid search: FTS5 + vector
│   │   └── src/
│   │       ├── lib.rs
...
│   │
│   ├── sigil-gates/                   # Messaging channels
│   │   └── src/
│   │       ├── lib.rs
...
│   │
│   ├── leader/                        # Lead Agent / Orchestrator
...
│   │
│   ├── advisor-a/                           # Advisor (project-alpha)
│   │   ├── PERSONA.md, IDENTITY.md, AGENTS.md
│   │   ├── KNOWLEDGE.md, MEMORY.md, PREFERENCES.md
│   │   └── 
│   │
│   ├── advisor-b/                           # Advisor (project-beta, project-gamma)
│   │   └── (same structure as advisor-a)
│   │
│   └── advisor-c/                      # Advisor (project-delta, silent)
│       └── (same structure as advisor-a)
│
...
│   │
│   ├── project-alpha/                    # Example project (Rust microservices)
│   │   ├── AGENTS.md                   # Operating instructions
│   │   ├── KNOWLEDGE.md               # Domain knowledge
│   │   ├── HEARTBEAT.md               # Health check config
│   │   ├── skills/                     # 10 project skills
│   │   ├── pipelines/                  # 2 project pipelines
│   │   ├── .tasks/                     # Task store (JSONL created at runtime)
│   │   └── .sigil/                     # Runtime (checkpoints, memory DB)
│   │
│   ├── project-beta/                 # Example project (Next.js)
│   │   ├── AGENTS.md, KNOWLEDGE.md, HEARTBEAT.md
│   │   ├── skills/, pipelines/
│   │   ├── .tasks/
│   │   └── .sigil/
│   │
│   ├── project-gamma/                   # Example project
│   │   ├── AGENTS.md, HEARTBEAT.md
│   │   ├── skills/, pipelines/
│   │   ├── .tasks/
│   │   └── .sigil/
│   │
│   └── project-delta/                   # Example project
│       ├── AGENTS.md
│       ├── skills/
│       └── .tasks/
│
...
```rust
let _new_config = SigilConfig::discover()...;
...
**Current state**: `ProjectConfig` has a `worktree_root` field, and `GitWorktreeTool` exists in `sigil-tools`, but the supervisor does NOT automatically create per-worker worktrees. Workers all share the same working directory.
...
**Problem**: Workers run as batch processes via `cmd.output()`. There is no intermediate progress reporting — the supervisor sees nothing until the worker completes (which can take up to 60 minutes for project-alpha).
...
**Fix direction**: Use `estimate_cost()` from `sigil-providers` with an estimated token count (based on task description length + context size) to predict cost before spawning. Skip tasks that would exceed remaining budget.
...
**Current state**: Exported from `sigil-orchestrator` lib.rs. Not referenced in any other module.
...
#### Gap 8: Conversation History Not Persisted

**Problem**: There is no persistent conversation history for Telegram interactions with the orchestrator. Each interaction starts fresh without prior context.

**Impact**: Telegram conversations with the daemon have no history persistence.

**Fix direction**: Add a conversation store to the daemon's Telegram handler. Record incoming/outgoing messages and build conversation context for the lead agent's system prompt.
