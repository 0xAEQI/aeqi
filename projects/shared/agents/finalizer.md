...
1. Load domain knowledge for this phase:
   - `sigil_skills(action="list", phase="finalize", project=<project>)` — list available skills
   - `sigil_skills(action="get", name=<relevant skill>)` — load any that match
...
4. Extract non-obvious learnings and store them:
   - `sigil_remember(project=<project>, key=<slug>, content=<learning>, category="fact"|"procedure")` — for each non-obvious discovery
5. Check for loose ends
6. Post report: `sigil_blackboard(action="post", project, key="task:<id>:complete", content=<report>)`
...
**Learnings**: Non-obvious discoveries, decisions, constraints — stored via `sigil_remember`.
